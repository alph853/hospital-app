...
function AddExamination() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { url, options } = await routes.addExamination(
      parseInt(patientId),
      parseInt(formData.doctor.ecode),
      parseInt(formData.fee.replace(/\D/g, "")),
      new Date(formData.e_date),
      formData.diagnosis,
      new Date(formData.next_exam_date)
    )
    if (!options) {
      navigate("/login")
      return
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const { url, options } = await routes.getDoctorsByDepartment(
        formData.department
      )
      if (!options) {
        navigate("/login")
      ...
      <form id="add" onSubmit={handleSubmit}>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, e_date: e.target.value })
            }
          />
        </div>
        <div>
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setFormData({ ...formData, next_exam_date: e.target.value })
            }
          />
        </div>
        {loading ? (
