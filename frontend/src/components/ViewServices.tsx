...
function ViewServices({
  ...
  const isTreatment = (service: Service): service is Treatment => {
    return (service as Treatment).start_date !== undefined
  }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.head}>
        <QuickFind
          onSubmit={searchService}
          placeholder={"Enter ID/Doctor name/Date"}
        />
        {type !== undefined && (
          <div className={styles.buttons}>
            {(type === 1 || type === 2) && (
              <Link to={`/patient/${patientId}/add/admission`}>
                <FontAwesomeIcon icon={faPlusCircle} />
                Add Admission
              </Link>
            )}
            {(type === 0 || type === 2) && (
              <Link to={`/patient/${patientId}/add/examination`}>
                <FontAwesomeIcon icon={faPlusCircle} /> Add Examination
              </Link>
            )}
          </div>
        )}
        ...
        <div className={styles.search}>
          ...
          {services.length > 0 &&
            currentServices.map((service) => (
              <div
                key={service.sid}
                className={`${styles.result} ${styles.hover} ${
                  selectedService === service.sid ? styles.selected : ""
                }`}
                onClick={() => {
                  onClick(service)
                }}
              >
                ...
                <div className={styles.doctors}>
                  {isTreatment(service) ? (
                    ...
                    <Link
                      to={`/doctor/${service.did.ecode}`}
                    >{`${service.did.ecode} - ${service.did.name}`}</Link>
                  )}
                </div>
                <div>{isTreatment(service) ? "idk" : service.fee}</div>
